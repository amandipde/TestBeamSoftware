...
//    TBranch *branch = telchain_->GetBranch("event");
      telchain_->SetBranchAddress("event", &telEvent_);  
  }
}

// Offline Cluster maker 
...
dutchain_->SetBranchAddress("L1A", &L1A);


// Start the Event Loop
 for (Long64_t jentry=0; jentry<nDutchainentry_;jentry++) {
...
//cout<<temphits_0.row_<<endl;
}

dtemp.dutHits.insert({"first", temphits_0}); //Insert Hit 0

for (int j = 0 ; j < hit1->size(); ++j) {//hit 1 loop
 temphits_1.emplace_back(tbeam::hit((*hit1)[j],0,(*hit1)[j] ,0,(*hit1)[j] ,1));
}
dtemp.dutHits.insert({"second", temphits_1}); //Insert Hit 1
...
tempclus_1 = offlineclusterizer(temphits_1, ev_.numberOfCBC, 127,tempclus_1);
  ...
  dtemp.offlineClusters.insert({"first", tempclus_0});
  dtemp.offlineClusters.insert({"second", tempclus_1});
...
dtemp.offlineStubs.insert({"second",tempStubs});
 ...
 dtemp.event = L1A;
  if(telFound_ && trigTrackmap_->find(dtemp.event) == trigTrackmap_->end())      continue;
	...
	tbeam::Track trkTemp(telOutputEvent_->xPos,telOutputEvent_->yPos,
		  	     ...
		  	     telOutputEvent_->xPosErrPrevHit,
			     telOutputEvent_->yPosErrPrevHit,
